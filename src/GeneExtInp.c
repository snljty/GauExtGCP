...
int main(int argc, char const * argv[])
{
  ...
  /*  acquire Gaussian level from ExtGCP_settings.ini  */
  iflp = fopen("ExtGCP_settings.ini", "r");
  while (! feof(iflp))
  {
    fgets(line, BUFSIZ, iflp);
    if (strstr(line, "the level without gCP in Gaussian format"))
    {
      fgets(line, BUFSIZ, iflp);
  ...
  /*  Generate Gaussian and gCP Input Files  */
  iflp = fopen(argv[1], "r");
  oflp1 = fopen("useExtGau.gjf", "w");
  oflp2 = fopen("useExtgCP.xyz", "w");
  fgets(line, BUFSIZ, iflp);
